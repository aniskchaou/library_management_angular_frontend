...
export class ShelfViewerComponent implements OnInit {
  ...
  departments: any[];
  private addedShelves: THREE.Sprite[] = [];
private addedBooks: THREE.Sprite[] = [];
...
private mouse = new THREE.Vector2();
  ...
  constructor(private router:Router,private libraryService: HTTPService) {
    ...
    this.fontLoader = new THREE.FontLoader();
     this.libraryService.getAll(CONFIG.URL_BASE+'/department/all').subscribe(
      (data:any[]) => {
        this.departments = data; // Assuming the response is an array of departments
        //this.clearScene()
        //this.showShelves(this.departments[0].departmentCode)
      },
      (error) => {
  ...
  showShelves(selectedDepartment){
    this.libraryService
  ...
  private addShelvesToScene(): void {
    ...
    this.addDepartmentLabel(this.selectedDepartment);
  ...
  private addShelf(shelf: any, index: number): void {
    ...
    this.addedShelves.push(shelfSprite); // Store the added shelf for future cleanup
  ...
  private addBook(book: any, bookIndex: number, shelfX: number, shelfY: number): void {
    ...
    this.loader.load(CONFIG.URL_BASE+'/book/get/'+book.id+'/'+book.imageUrl, (texture) => {
      ...
      this.addedBooks.push(bookSprite);
 ...
 */
  ...
  private addDepartmentLabel(departmentName: string): void {
    ...
    const dep=this.departments.find(x=>x.departmentCode===departmentName)
    const loader = new THREE.FontLoader();
    loader.load('assets/fonts/helvetiker_regular.typeface.json', (font) => {
        const textGeometry = new THREE.TextGeometry(dep.departmentName, {
            font: font,
        ...
        // Position the label at the top of the scene
        const labelHeight = 80; // Set this height above the shelves
        mesh.position.set(0, labelHeight, 0); // Adjust y for top positioning
        mesh.rotation.x = -Math.PI / 2; // Optional: Rotate the label to face the camera
